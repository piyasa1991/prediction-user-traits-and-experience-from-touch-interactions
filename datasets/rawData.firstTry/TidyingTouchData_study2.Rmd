---
title: "Tidying Touch Interaction Data Giorgio's Study"
...
setwd("~/Auswertungen/2017-07_Studie_Giorgio/FeatureEngineering")
...
VP.numbers <- as.vector(c(611:671))
User.ids <- as.vector(c(190:250))
```
...
if ( !file.exists("01_DataExperiment_unprocessed.Rda") ) {
  ...
  # data.experiment <- data.experiment.interactions[data.experiment.interactions$user_id %in% User.ids,]
...
if ( !file.exists("02_DataExperiment_labeledWithConds+SessionNrs.Rda") ) {
  ...
  data.experiment$App <- factor(data.experiment$appCode, labels = c("Spell"))
  ...
  data.experiment$condCode[data.experiment$freezing == 0 & data.experiment$tinyIcons == 0] <- 1
  data.experiment$condCode[data.experiment$freezing == 0 & data.experiment$tinyIcons == 1] <- 2
  data.experiment$Cond <- factor(data.experiment$condCode, labels = c("noInfo", "Normal", "TinyIcons"))
  ...
  #######################################################
  # add study-typical conditions
  #######################################################
  
  library(readr)
  mapping.sessions.conditions <- read_delim("../Data_Demo_Mapping/SessionNr_Condition-Table 1.csv", 
    ";", escape_double = FALSE, trim_ws = TRUE)
  test <- merge(data.experiment, mapping.sessions.conditions, by.x = c("user_id", "sessionNr"), by.y = c("UserId", "SessionNr"))
  test$Size[test$Size == "Small"] <- "TinyIcons"  
  test$Size[test$Size == "Wrong Participant"] <- "noInfo"  
  test$labeling.mistake <- FALSE
  test <- test[is.na(test$Command) | test$Command != "Wrong Participant",]
  
  test$Position <- as.factor(test$Position)
  test$HandUsed <- as.factor(test$HandToDrag)
  test$Handedness[test$Handedness == "Left"] <- "LEFT"
  test$Handedness[test$Handedness == "Right"] <- "RIGHT"
  test$Handedness <- as.factor(test$Handedness)
         
  table(test$Size[test$Size != "noInfo"] == test$Cond[test$Size != "noInfo"])
  incorrect.labeled.sessions <- unique(test$sessionNr[(test$Size[test$Size != "noInfo"] != test$Cond[test$Size != "noInfo"])])
  # incorrect.labeled.participant <- unique(test$user_id[(test$Size[test$Size != "noInfo"] != test$Cond[test$Size != "noInfo"])])
  
  for (s in incorrect.labeled.sessions) {
    test$labeling.mistake[test$sessionNr == s] <- TRUE
    test$Size[test$sessionNr == s] <- as.character(test$Cond[test$sessionNr == s])
  }
  
  table(test$Size[test$Size != "noInfo"] == test$Cond[test$Size != "noInfo"])
  
  test$IconSize <- factor(test$Size, labels = c("Normal", "TinyIcons"))
  
  test$Cond <- interaction(test$IconSize, test$Position, test$Handedness, test$HandUsed)
  
  data.experiment <- test
  
  test <- data.experiment
  
  barplot(table(data.experiment$Cond))
  
  length(unique(data.experiment$sessionNr))
  
  sessions.to.merge <- unique(test$sessionNr[!is.na(test$Command)])
  for (s in sessions.to.merge){
    test$sessionNr[test$sessionNr == (s + 1)] <- s
  }
  
  length(unique(test$sessionNr))
  test$Command <- NULL
  
  data.experiment <- test
  data.experiment$condCode <- as.factor(data.experiment$Condition)
  
  # unique(test$Size[test$sessionNr == 104 ])
  # unique(test$Cond[test$sessionNr == 104 ])
  # 
  # unique(test$Size[test$sessionNr == 139])
  # unique(test$Cond[test$sessionNr == 139 ])
  # 
  # unique(test$Size[test$sessionNr == 245])
  # unique(test$Cond[test$sessionNr == 245])
  # 
  # unique(test$Size[test$sessionNr == 246 ])
  # unique(test$Cond[test$sessionNr == 246])
